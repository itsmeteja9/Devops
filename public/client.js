...
const activity = [
  { stage: 0, time: '00:01', text: 'Checked out main at commit 5f1c1d5' },
  { stage: 1, time: '00:34', text: 'Unit tests passed — 92% line coverage' },
  { stage: 1, time: '00:47', text: 'SonarQube SAST and Trivy repository scan passed' },
  ...
  { stage: 3, time: '03:05', text: 'Built and scanned image: helm-demo:5f1c1d5' },
  { stage: 3, time: '03:24', text: 'Published approved image to Artifact Registry' },
  { stage: 4, time: '04:42', text: 'GKE deployment helm-demo-xxxxxx is ready' },
...
function renderStages() {
  elements.stageGrid.innerHTML = stages
    .map((stage, index) => {
      ...
      return `
        <button
          class="stage stage-${stage.color} ${selected ? 'is-selected' : ''} status-${status}"
          type="button"
          data-stage="${index}"
...
function renderResponsibilities() {
  elements.responsibilityGrid.innerHTML = stages
    .map(
      (stage, index) => `
        <article
          class="responsibility-item ${state.activeStage === index ? 'is-active' : ''}"
...
function renderActivity() {
  elements.activityLog.innerHTML = activity
    .map((entry) => {
      ...
      return `
        <div class="log-entry ${visible ? 'success' : ''} ${!visible ? 'info' : ''}">
          <span>${marker}</span> <time>${entry.time}</time> ${text}
        </div>
      `;
    })
    .join('');
...
function renderSummary() {
  ...
  elements.summaryNumber.className = `summary-number`;
...
function setServiceView(mode) {
  ...
  if (mode === 'deploying') {
    ...
    label.textContent = 'GKE DEPLOYMENT';
  ...
  label.textContent = 'RUNNING ON GKE';
  headline.innerHTML = 'It shipped. <span>✓</span>';
  elements.revisionCopy.textContent = 'This page is the application currently served by your GKE cluster.';
}
